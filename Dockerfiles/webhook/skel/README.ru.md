...
, который формирует файл .config вида `ключ=значение`
...
**ВАЖНО**: для использования данного документа необходимо наличие ssh-доступа к it.tender.pro,
который предоставляется по запросу после регистрации в [Gogs](https://git.it.tender.pro).
Предварительно надо загрузить открытый ключ ssh на [страницу](http://rm.tender.pro/projects/pwl-dev/files).
